...
if (defined $outputDir){
	...
	$outputDir = File::Spec -> catfile($wk_dir,"qap_Results_for_ConsensusSeq_$DateNow");
...
#check gs and seqlogo and set gs system PATH
my $DEBUG_MODE = 1;
my $gsProgram;
my $seqlogoProgram;
if($seqlogo){
	# check gs
	$gsProgram = File::Spec -> catfile($mainBin,'bin','3rdPartyTools','cs','gs','gs');
	if(CheckProgram($gsProgram, __FILE__, __LINE__, $DEBUG_MODE)){
		#keep running
	}else{
		InfoError("The program $gsProgram does NOT exist. Exiting...");
		exit;
	}
	
	# check seqlogo
	$seqlogoProgram = File::Spec -> catfile($mainBin,'bin','3rdPartyTools','cs','weblogo','seqlogo');
	if(CheckProgram($seqlogoProgram, __FILE__, __LINE__, $DEBUG_MODE)){
		#keep running
	}else{
		InfoError("The program $seqlogoProgram does NOT exist. Exiting...");
		exit;
	}
...
gap ConsensusSeq -i ./seq -t 10 -s fasta -g -o ./cutTile
