...
if (defined $outputDir){
	...
	$outputDir = File::Spec -> catfile($wk_dir,"qap_Results_for_ShannonEntroy_$DateNow");
...
#check r script 
my $rscript = File::Spec -> catfile($mainBin, 'bin', 'Rscripts', 'CalculateShannonEntroy.R');
if (-e $rscript){
	#nothing
}else{
...
my $resfileName = "ShannonEntroy" . "_" . $inputDirName . ".txt";
...
sub shannon {
	...
	if($wk_dir eq $outputDir){
		...
		my $tif = File::Spec -> catfile($wk_dir, "${fileName}.tif");
		$cmd = "mv $tif $outputDir";
		system($cmd);
...
gap ShannonEntroy -i ./seq -f fasta -s fas -g T -o ./shannon
