...
my $bamfile = $inputFile =~ s/\.sam$/.PosSorted.bam/r;

copy($bamfile,$outputFile);
...
This script implements a function to convert sam file to bam file. The script has B<several> mandatory options that MUST appear last. 
...
Path to input SAM file which is required. 
...
qap Bam2Sam -i test.sam -o test.bam -t 10
