...
par(mar=c(1,1,5,5))
image(1:xlen,1:ylen,t(t(cor2.half[,c(ncol(cor2):1)])),
      col=colorpanel(length(breaks) - 1,opt$heatColor,"orange","red"),
      #col=terrain.colors(length(breaks) - 1),
      breaks = breaks,
      axes = FALSE, ann=F)

axis(3,c(1:ncol(cor2)),samples,tick = F,las=2)
axis(4,c(1:nrow(cor2)),samples[nrow(cor2):1],tick = F,las=1)
abline(v = c(0.5:10.5),h=c(0.5:10.5),col="white")
