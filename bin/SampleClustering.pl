...
printcol ("SampleClustering","green");
print "\n";
...
if (defined $outputDir){
	...
	$outputDir = File::Spec -> catfile($wk_dir,"qap_Results_for_SampleClustering_$DateNow");
...
##start to calculate
#rscript
my $rscript = File::Spec -> catfile($RealBin,"Rscripts","SampleClustering.R");
...
#run r script
Info("Running sample clustering and drawing sample dendrogram tree.");
my $outfig = File::Spec -> catfile($outputDir,"SampleClustering");
my $cmd = "Rscript $rscript --inputFile $inputFile --group $group --outputFile $outfig";
runcmd($cmd);
...
This script implements a function to cluster samples and draw dendrogram tree. The script has B<several> mandatory options that MUST appear last. 
...
qap SampleClustering -i ./OTUTable.txt -g G1,G1,G2,G2,G2,G3,G3 -o ./result
